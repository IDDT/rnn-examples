...
class Encoder(nn.Module):
    def __init__(self, input_dim, z_dim):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, padding=1, stride=2), nn.ReLU(), #28 -> 14
            nn.Conv2d(32, 64, kernel_size=3, padding=1, stride=2), nn.ReLU(), #14 -> 7
            nn.Conv2d(64, 128, kernel_size=3, stride=2), nn.ReLU(), #7 -> 3
            nn.Conv2d(128, 256, kernel_size=3), nn.ReLU(), #3 -> 1
            nn.BatchNorm2d(256),
        )
        self.lin_mean = nn.Linear(256, z_dim)
        self.lin_log_sigma = nn.Linear(256, z_dim)
    def forward(self, x):
...
class Decoder(nn.Module):
    def __init__(self, z_dim, output_dim):
        super().__init__()
        self.lin_h = nn.Linear(z_dim, 256)
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(256, 128, kernel_size=3), nn.ReLU(), #1 -> 3
            nn.ConvTranspose2d(128, 64, kernel_size=3, stride=2), nn.ReLU(), #3 -> 7
            nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2), nn.ReLU(), #7 -> 14
            nn.ConvTranspose2d(32, 1, kernel_size=2, stride=2), nn.Sigmoid(), #28 -> 14
        )
    def forward(self, x):
        #Decode z into input.
        h = self.lin_h(x)
...
decoder = Decoder(z_dim, output_dim)
model = VAE(encoder, decoder).to(device)
optim = torch.optim.Adam(model.parameters())
# model.load_state_dict(torch.load('models/vae_conv_mnist.model', map_location=device))
# optim.load_state_dict(torch.load('models/vae_conv_mnist.optim', map_location=device))
...
for epoch in range(1000):
    ...
    for X, _ in dataloader_train:
        ...
        X_pred = X_pred.squeeze(1).reshape(-1, input_dim)
        X = X.squeeze(1).reshape(-1, input_dim)
    ...
    if loss_test < loss_min:
        torch.save(model.state_dict(), 'models/vae_conv_mnist.model')
        torch.save(optim.state_dict(), 'models/vae_conv_mnist.optim')
        loss_min = loss_test
        unimproved_epochs = 0
...
X = torch.cat(X, dim=0).unsqueeze(1).to(device)
with torch.no_grad():
    X_pred = model(X)[0]
