...
class Decoder(nn.Module):
    ...
    def forward(self, x, h, forcing_ratio=1):
...
# encoder.load_state_dict(torch.load('models/encoder_so.model', map_location=device))
# decoder.load_state_dict(torch.load('models/decoder_so.model', map_location=device))
# optim_enc.load_state_dict(torch.load('models/encoder_so.optim', map_location=device))
# optim_dec.load_state_dict(torch.load('models/decoder_so.optim', map_location=device))
...
for epoch in range(1001):
    ...
    for Xi, X, y in dataloader_train:
        ...
        h = encoder(Xi)
        y_pred = decoder(X, h, forcing_ratio=forcing_ratio)
    ...
    with torch.no_grad():
        for Xi, X, y in dataloader_test:
            ...
            Xi, X, y = Xi.to(device=device), X.to(device=device), y.to(device)
            h = encoder(Xi)
            y_pred = decoder(X, h, forcing_ratio=0.0)
    ...
    if loss_test < loss_min:
        torch.save(encoder.state_dict(), 'models/encoder_so.model')
        torch.save(decoder.state_dict(), 'models/decoder_so.model')
        torch.save(optim_enc.state_dict(), 'models/encoder_so.optim')
        torch.save(optim_dec.state_dict(), 'models/decoder_so.optim')
        loss_min = loss_test
        unimproved_epochs = 0
    if unimproved_epochs > max_unimproved_epochs:
        if forcing_ratio > 0:
            print(f'E {epoch} Decreasing forcing ratio. Resetting minimum loss.')
            forcing_ratio = max(0, forcing_ratio - 0.05)
            loss_min = float('inf')
            unimproved_epochs = 0
        else:
